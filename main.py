...
import cpfutils as cpfutil 
root = tk.Tk()


def validar(stringvar: tk.StringVar, button: tk.Button):
    cpf = stringvar.get()
...
def gerador(stringvar: tk.StringVar, button: tk.Button):
...
validate_button.configure(command=lambda: validar(
...
generate_button.configure(command=lambda: gerador(
